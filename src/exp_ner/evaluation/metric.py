...
class EpochStats:
    ...
    def metrics(self, index2label: Union[List[str], Dict[int, str]]):
        golds, preds =self._map_to_labels(index2label)

        f1 = f1_score(golds, preds, zero_division='0') * 100
        p = precision_score(golds, preds, zero_division='0') * 100
        r = recall_score(golds, preds, zero_division='0') * 100

        return f1, p, r

    def get_classification_report(self, index2label: Union[List[str], Dict[int, str]]):        
        golds, preds = self._map_to_labels(index2label)

        cr = classification_report(golds, preds, digits=5, zero_division='0')
        return report2dict(cr)

    def print_classification_report(self, index2label: Union[List[str], Dict[int, str]] = None, report = None):
